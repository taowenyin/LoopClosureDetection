...
class BackBone(nn.Module):
    def __init__(self, config):
        ...
        if config['arch_type'] == 'resnet50':
            deeplab = models.segmentation.deeplabv3_resnet50(pretrained=config.getboolean('pretrained'))
        elif config['arch_type'] == 'resnet101':
            deeplab = models.segmentation.deeplabv3_resnet101(pretrained=config.getboolean('pretrained'))
        elif config['arch_type'] == 'mobilenet':
            deeplab = models.segmentation.deeplabv3_mobilenet_v3_large(pretrained=config.getboolean('pretrained'))
        else:
            deeplab = models.segmentation.deeplabv3_mobilenet_v3_large(pretrained=config.getboolean('pretrained'))
